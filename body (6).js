    ...
    try {
        ...
        if (!fs.existsSync(credsPath) || conf.session !== "zokk") {
            ...
            if (/^[a-zA-Z0-9]+_[a-z0-9]{5}$/i.test(conf.session)) {
                const sessionUrl = `https://session-store.pages.dev/${conf.session}.json`;
            ...
            // Handle direct BWM-XMD;;; session string
            else if (conf.session.startsWith('bravin;;;')) {
                const [header, b64data] = conf.session.split(';;;');
                
                if (header !== "bravin" || !b64data) {
                ...
                throw new Error("Invalid session format - must be [name]_xxxxx or Jeepers-creeper;;; format");
            }
        }
    } catch (error) {
...
const handleDeletedMessage = async (deletedMsg, key, deleter) => {
    const context = createContext(deleter, {
        ...
        thumbnail: "https://files.catbox.moe/jndzlw.jpg"
    });
...
if (conf.AUTO_BIO === "yes") {
    const updateBio = async () => {
        try {
            ...
            const bioText = `jeepers creeper bot developed by sir bravin\n➤ ${quote}\n📅 ${timeDate}`;
            ...
            if (statusUpdates.length > 0) {
                ...
                if (now - lastNotification > 300000) { // 5 minutes cooldown
                    ...
                    await adams.sendMessage(statusSender, {
                        text: `${conf.REPLY_STATUS_TEXT || "*ʏᴏᴜʀ sᴛᴀᴛᴜs ʜᴀᴠᴇ ʙᴇᴇɴ ᴠɪᴇᴡᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ ✅*"}\n\n📌 For more info visit: *https://sir-bravine-delta.vercel.app/*\n\n> sir bravin the developer`,
                        contextInfo: {
...
const welcomeImage = 'https://files.catbox.moe/jndzlw.jpg';
const goodbyeImage = 'https://files.catbox.moe/jndzlw.jpg';
    ...
    try {
        ...
        try {
            ...
            if (isIdentityQuestion) {
                aiResponse = 'I am jëëpërs çrëëpër bot,created by sir bravin! 🚀';
            }
            ...
            return isIdentityQuestion 
                ? `I am jëëpërs çrëëpër bot, created by sir bravin! 🚀`
                : textResponse;
        }
    } catch (error) {
        console.error("API Error:", error);
        return isIdentityQuestion 
            ? "I'm jëëpërs çrëëpër bot, created by sir bravin! 🚀"
        ...
        try {
            ...
            if (conf.CHATBOT === "yes") {
                await adams.sendMessage(jid, { 
                    ...
                    ...createContext(jid, {
                        title: "jëëpërs çrëëpër conversation ",
                        body: "ᴘᴏᴡᴇʀᴇᴅ ʙʏ SIR BRAVIN"
                    })
                }, { quoted: msg });
            }
            ...
            if (conf.CHATBOT1 === "yes") {
                ...
                if (ttsText) {
                    ...
                    await adams.sendMessage(jid, {
                        ...
                        ptt: true,
                        ...createContext2(jid, {
                            title: "jëëpërs çrëëpër ᴀᴜᴅɪᴏ_ᴄʜᴀᴛʙᴏᴛ",
                            body: "ᴘᴏᴡᴇʀᴇᴅ ʙʏ sir bravin"
                        })
                    }, { quoted: msg });
                }
...
adams.ev.on('messages.upsert', async (msg) => {
    ...
    if (connection === "open") {
        console.log("🌎 jeepers creeper is ONLINE 🌎");
        reconnectAttempts = 0;
        
        setTimeout(async () => {
            try {
                console.log('🚀 Enjoy Jeepers creeper bot speed 🌎');
...
└─────────────────┈ ⳹`;
                    ...
                    await adams.sendMessage(
                        ...
                        {
                            text: connectionMsg,
                            ...createContext("sir bravin", {
                                title: "SYSTEM ONLINE",
                                body: "jëëpërs çrëëpër"
                            })
                        },
                        {
